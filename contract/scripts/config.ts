...
export const PREPROD_CONFIG: NetworkConfig = {
  ...
  proofServer: process.env['MIDNIGHT_PROOF_SERVER'] ?? 'http://127.0.0.1:6300',
  faucet: 'https://midnight-tmnight-testnet.nethermind.dev/',
};
...
export function getConfig(): NetworkConfig {
  ...
  if (network === 'testnet' || network === 'preprod') return PREPROD_CONFIG;
  throw new Error(
    `Unknown network: ${network}. Supported: 'local', 'preview', 'testnet'.`,
  );
}
