...
- **Privacy Design**: The contract uses the `UserAttributes` as a private `witness`, enforcing that the user's secret identity, age, and issuer signature remain off-chain in the shielded vault. The ZK Circuit (`verify_and_claim`) selectively discloses that the user met the criteria without revealing the underlying data, while generating a zero-knowledge proof. A `nullifier` is published to the ledger to prevent replay attacks without linking back to the user's credential.
