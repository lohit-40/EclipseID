...
EclipseID leverages Midnight to fill a massive gap in the ecosystem:
1. **Sybil-Resistant Airdrops**: Protocols can verify unique humanity without storing biometric data on-chain.
2. **Permissioned DeFi / RWA**: Institutions can enforce KYC/AML compliance while preserving user privacy and trade secrecy. **(Our contract now strictly enforces ZK Accreditation Checks).**
3. **Age-Gating**: Smart contracts can enforce age limits (e.g., > 18) by simply verifying the proof, without learning the user's actual birth date. **(Our contract now enforces `age >= 18` in the `verify_and_claim` circuit).**
...
- [x] **Demo video showing full MVP functionality:** [YouTube Video](https://youtu.be/qKA7nbQtTvc)
- [x] **Minimum 20 meaningful commits:** Completed (currently 90+ meaningful commits).
